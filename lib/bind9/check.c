...
static isc_result_t
check_forward(const cfg_obj_t *options,
...
static isc_result_t
mustbesecure(const cfg_obj_t *secure,
	     isc_symtab_t *symtab,
	     isc_log_t *logctx,
	     isc_mem_t *mctx)
{
...
static isc_result_t
check_viewacls(cfg_aclconfctx_t *actx,
	       const cfg_obj_t *voptions,
	       const cfg_obj_t *config,
	       isc_log_t *logctx,
	       isc_mem_t *mctx)
...
static isc_result_t
check_dns64(cfg_aclconfctx_t *actx,
	    const cfg_obj_t *voptions,
	    const cfg_obj_t *config,
	    isc_log_t *logctx,
	    isc_mem_t *mctx)
...
static isc_result_t
check_ratelimit(cfg_aclconfctx_t *actx,
		const cfg_obj_t *voptions,
		const cfg_obj_t *config,
		isc_log_t *logctx,
		isc_mem_t *mctx)
 ...
 */
static isc_result_t
check_recursionacls(cfg_aclconfctx_t *actx,
		    const cfg_obj_t *voptions,
		    const char *viewname,
		    const cfg_obj_t *config,
		    isc_log_t *logctx,
...
static isc_result_t
check_options(const cfg_obj_t *options,
	      isc_log_t *logctx,
	      isc_mem_t *mctx,
	      optlevel_t optlevel)
{
...
static isc_result_t
validate_masters(const cfg_obj_t *obj,
		 const cfg_obj_t *config,
		 uint32_t *countp,
		 isc_log_t *logctx,
		 isc_mem_t *mctx)
 ...
 */
static bool
check_mirror_zone_notify(const cfg_obj_t *zoptions,
			 const char *znamestr,
			 isc_log_t *logctx)
{
 ...
 */
static bool
check_recursion(const cfg_obj_t *config,
		const cfg_obj_t *voptions,
		const cfg_obj_t *goptions,
		isc_log_t *logctx,
		cfg_aclconfctx_t *actx,
		isc_mem_t *mctx)
{
...
static isc_result_t
check_zoneconf(const cfg_obj_t *zconfig,
	       const cfg_obj_t *voptions,
	       const cfg_obj_t *config,
	       isc_symtab_t *symtab,
	       isc_symtab_t *files,
	       ...
	       isc_mem_t *mctx)
{
	...
	if (tresult != ISC_R_SUCCESS) {
		...
		tresult = nameexist(zconfig, namebuf,
				    ...
				    "previous definition: %s:%u", logctx,
				    mctx);
		if (tresult != ISC_R_SUCCESS) {
			result = tresult;
		}
		...
		case CFG_ZONE_STATICSTUB:
			...
			if (tmp != NULL) {
				...
				tresult = isc_symtab_define(inview,
							    tmp,
							    1,
							    symvalue,
...
bind9_check_key(const cfg_obj_t *key, isc_log_t *logctx) {
	...
	if (algorithm[len] == '-') {
		uint16_t digestbits;
		result =
			isc_parse_uint16(&digestbits, algorithm + len + 1, 10);
	  ...
	  isc_log_t *logctx)
{
	...
	result =
		isc_symtab_lookup(symtab, cfg_obj_asstring(obj), 0, &symvalue);
	if (result == ISC_R_SUCCESS) {
		if (writeable) {
			file = cfg_obj_file(symvalue.as_cpointer);
			line = cfg_obj_line(symvalue.as_cpointer);
 ...
 */
static isc_result_t
check_keylist(const cfg_obj_t *keys,
	      isc_symtab_t *symtab,
	      isc_mem_t *mctx,
	      isc_log_t *logctx)
{
...
static isc_result_t
check_servers(const cfg_obj_t *config,
	      const cfg_obj_t *voptions,
	      isc_symtab_t *symtab,
	      isc_log_t *logctx)
{
	...
	for (e1 = cfg_list_first(servers); e1 != NULL;
	     e1 = cfg_list_next(e1))
	{
...
static isc_result_t
check_trusted_key(const cfg_obj_t *key,
		  bool managed,
...
static isc_result_t
check_rpz_catz(const char *rpz_catz,
 ...
 */
static isc_result_t
check_one_plugin(const cfg_obj_t *config,
		 const cfg_obj_t *obj,
...
static isc_result_t
check_viewconf(const cfg_obj_t *config,
	       const cfg_obj_t *voptions,
	       const char *viewname,
...
static isc_result_t
bind9_check_controlskeys(const cfg_obj_t *control,
...
static isc_result_t
bind9_check_controls(const cfg_obj_t *config,
		     isc_log_t *logctx,
		     isc_mem_t *mctx)
{
	isc_result_t result = ISC_R_SUCCESS, tresult;
