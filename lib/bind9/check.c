	      ...
	      optlevel_t optlevel)
{
	...
	if (obj != NULL) {
		if (optlevel != optlevel_zone && optlevel != optlevel_config) {
			cfg_obj_log(obj, logctx, ISC_LOG_ERROR,
				    "dnssec-policy may only be activated at "
				    "the top level and referenced at the "
				    "zone level");
			if (result == ISC_R_SUCCESS) {
				result = ISC_R_FAILURE;
			}
	       ...
	       cfg_aclconfctx_t *actx, isc_log_t *logctx, isc_mem_t *mctx)
{
	...
	if (ztype == CFG_ZONE_MASTER || ztype == CFG_ZONE_SLAVE) {
		...
		/*
		 * WMM: Maybe don't allow dnssec-loadkeys-interval and
		 * dnssec-policy at the same time.
		 */
