...
#define atomic_store_explicit32(obj,desired,order)            \
	(order == memory_order_relaxed                          \
	 ...
	 : (order == memory_order_acquire                       \
	    ? (void)InterlockedExchangeAcquire((atomic_int_fast32_t*)obj, \
					       desired) \
...
#define atomic_store_explicit64(obj,desired,order)            \
	(order == memory_order_relaxed                          \
	 ? (void)InterlockedExchangeNoFence64((atomic_int_fast64_t*)obj, \
					      desired) \
	 : (order == memory_order_acquire                       \
	    ? (void)InterlockedExchangeAcquire64((atomic_int_fast64_t*)obj, \
						 desired) \
					 ...
					 memory_order fail)
{
	...
	int8_t __v;
	REQUIRE(succ == fail);
	__v = InterlockedCompareExchange8((atomic_int_fast8_t*)obj,
					  desired,
					  *expected);
	__r = (*(expected) == __v);
	if (!__r) {
