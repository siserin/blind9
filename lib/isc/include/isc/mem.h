...
#define ISC_MEMFLAG_FILL        0x00000004       /* fill with pattern after
	                                          * alloc and frees */
...
#define isc_mem_allocate(c, s)  ISCMEMFUNC(allocate)((c), \
						     (s) _ISC_MEM_FILELINE)
#define isc_mem_reallocate(c, p, s) ISCMEMFUNC(reallocate)((c), (p), \
							   (s) \
							   _ISC_MEM_FILELINE)
...
/*@{*/
isc_result_t
isc_mem_create(size_t max_size, size_t target_size, isc_mem_t **mctxp);

isc_result_t
isc_mem_createx(size_t max_size,
		size_t target_size,
