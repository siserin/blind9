...
#define XTRACETIME2(s,d,n)            fprintf(stderr, \
					      "%s %u.%09u %u.%09u\n", \
					      (s), \
					      (d).seconds, \
					      (d).nanoseconds, \
...
schedule(isc__timer_t*timer,isc_time_t*now,bool signal_ok) {
	...
	if (timer->type != isc_timertype_once) {
		...
		if (isc_time_isepoch(&timer->idle)) {
			...
			due = timer->idle;
		} else if (isc_time_compare(&timer->idle,
					    &timer->expires) < 0)
		{
			due = timer->idle;
		} else {
...
isc_result_t
isc_timer_create(isc_timermgr_t*manager0,
		 isc_timertype_t type,
...
isc_result_t
isc_timer_reset(isc_timer_t*timer0,
		isc_timertype_t type,
		const isc_time_t*expires,
		const isc_interval_t*interval,
		bool purge)
...
run(void*uap) {
	...
	while (!manager->done) {
		...
		if (manager->nscheduled > 0) {
			...
			result = WAITUNTIL(&manager->wakeup,
					   &manager->lock,
					   &manager->due);
