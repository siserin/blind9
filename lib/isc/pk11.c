...
isc_result_t
pk11_get_session(pk11_context_t *ctx,
...
isc_result_t
pk11_parse_uri(pk11_object_t *obj,
	       const char *label,
	       isc_mem_t *mctx,
	       pk11_optype_t optype)
{
		...
		if ((a == v) || (strcmp(a, "object") == 0)) {
			...
			if (token == NULL) {
				for (token = ISC_LIST_HEAD(tokens);
				     token != NULL;
				     token = ISC_LIST_NEXT(token, link))
					if (pk11strcmp(v, l, token->serial,
						       16)) {
						break;
					}
			}
...
void
pk11_error_fatalcheck(const char *file,
		      int line,
		      const char *funcname,
		      CK_RV rv)
{
