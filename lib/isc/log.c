 ...
 */
static isc_result_t
assignchannel(isc_logconfig_t *lcfg,
...
isc_result_t
isc_log_createchannel(isc_logconfig_t *lcfg,
		      const char *name,
		      unsigned int type,
		      int level,
...
isc_result_t
isc_log_usechannel(isc_logconfig_t *lcfg,
		   const char *name,
...
void
isc_log_vwrite1(isc_log_t *lctx,
		isc_logcategory_t *category,
		isc_logmodule_t *module,
		int level,
		const char *format,
	     ...
	     va_list args)
{
	...
	do {
		...
		printcolon    = ((channel->flags & ISC_LOG_PRINTTAG) != 0 &&
				 lcfg->tag != NULL);
		printcategory =
			((channel->flags & ISC_LOG_PRINTCATEGORY) != 0);
