...
static void
update_log(dns_update_log_t *callback,
	   dns_zone_t *zone,
	   int level,
	   const char *fmt,
 ...
 */
static isc_result_t
non_nsec_rrset_exists(dns_db_t *db,
		      dns_dbversion_t *ver,
		      dns_name_t *name,
		      bool *exists)
{
 ...
 */
static isc_result_t
next_active(dns_update_log_t *log,
	    dns_zone_t *zone,
	    dns_db_t *db,
	    dns_dbversion_t *ver,
	    dns_name_t *oldname,
 ...
 */
static isc_result_t
add_placeholder_nsec(dns_db_t *db,
		     dns_dbversion_t *ver,
		     dns_name_t *name,
		     dns_diff_t *diff)
{
...
static isc_result_t
find_zone_keys(dns_zone_t *zone,
	       dns_db_t *db,
	       dns_dbversion_t *ver,
	       isc_mem_t *mctx,
...
static isc_result_t
add_exposed_sigs(dns_update_log_t *log,
		 dns_zone_t *zone,
		 dns_db_t *db,
		 dns_dbversion_t *ver,
		 dns_name_t *name,
 ...
 skip:
	...
	case build_chain:
		     ...
		     t = ISC_LIST_NEXT(t, link))
		{
			...
			CHECK(namelist_append_name(&state->affected,
						   prevname));
		}
