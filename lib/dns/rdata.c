...
static isc_result_t
rdata_totext(dns_rdata_t *rdata,
	     dns_rdata_textctx_t *tctx,
	     isc_buffer_t *target);

static void
warn_badname(const dns_name_t *name,
	     isc_lex_t *lexer,
	     dns_rdatacallbacks_t *callbacks);

static void
warn_badmx(isc_token_t *token,
	   isc_lex_t *lexer,
	   dns_rdatacallbacks_t *callbacks);
...
static isc_result_t
unknown_totext(dns_rdata_t *rdata,
	       dns_rdata_textctx_t *tctx,
	       isc_buffer_t *target);
...
static inline isc_result_t
name_duporclone(const dns_name_t *source, isc_mem_t *mctx,
		dns_name_t *target) {
	if (mctx != NULL) {
...
static inline isc_result_t
typemap_totext(isc_region_t *sr,
	       dns_rdata_textctx_t *tctx,
	       isc_buffer_t *target)
{
	unsigned int i, j, k;
		...
		for (j = 0; j < len; j++) {
			...
			for (k = 0; k < 8; k++) {
				...
				if (dns_rdatatype_isknown(t)) {
					...
					snprintf(buf, sizeof(buf), "TYPE%u",
						 t);
 ...
 */
static isc_result_t
rdata_validate(isc_buffer_t *src,
	       isc_buffer_t *dest,
	       dns_rdataclass_t rdclass,
	       dns_rdatatype_t type)
{
...
static isc_result_t
unknown_fromtext(dns_rdataclass_t rdclass,
		 dns_rdatatype_t type,
		 isc_lex_t *lexer,
		 isc_mem_t *mctx,
		 isc_buffer_t *target)
...
btoa_totext(unsigned char *inbuf, int inbuflen, isc_buffer_t *target) {
	 ...
	 * delimited by 'x'
	 */
	snprintf(buf, sizeof(buf), "x %d %x %x %x", inbuflen, Ceor, Csum,
		 Crot);
	return (str_totext(buf, target));
}
...
dns_rdatatype_atcname(dns_rdatatype_t type) {
	if ((dns_rdatatype_attributes(type) & DNS_RDATATYPEATTR_ATCNAME) !=
	    0) {
		return (true);
	}
	return (false);
