...
unsigned int
dns_nsec_compressbitmap(unsigned char *map,
...
isc_result_t
dns_nsec_buildrdata(dns_db_t *db,
...
dns_nsec_typepresent(dns_rdata_t *nsec, dns_rdatatype_t type) {
	...
	for (i = 0; i < nsecstruct.len; i += len) {
		...
		if (type < (window * 256) + len * 8) {
			present =
				dns_nsec_isset(&nsecstruct.typebits[i],
					       type % 256);
		}
		break;
	}
 ...
 */
isc_result_t
dns_nsec_noexistnodata(dns_rdatatype_t type,
		       const dns_name_t *name,
		       const dns_name_t *nsecname,
