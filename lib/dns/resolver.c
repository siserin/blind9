...
static void
rctx_nextserver(respctx_t *rctx,
		dns_adbaddrinfo_t *addrinfo,
		isc_result_t result);
...
static void
fctx_cancelquery(resquery_t **queryp,
...
compute_cc(resquery_t *query, unsigned char *cookie, size_t len) {
	...
	memmove(cookie, digest, 8);
#endif /* if defined(HMAC_SHA1_CC) || defined(HMAC_SHA256_CC) */
}

static isc_result_t
issecuredomain(dns_view_t *view,
...
fctx_try(fetchctx_t *fctx, bool retrying, bool badcache) {
	...
	if (fctx->minimized && !fctx->forwarding) {
		...
		fctx_stoptimer(fctx);
		result = dns_resolver_createfetch(fctx->res,
						  &fctx->qminname,
						  fctx->qmintype,
						  &fctx->domain,
						  &fctx->nameservers,
	  ...
	  dns_fetch_t *fetch)
{
	...
	event = (dns_fetchevent_t *)
		isc_event_allocate(fctx->res->mctx,
				   tclone,
				   DNS_EVENT_FETCHDONE,
				   action,
				   arg,
...
static isc_result_t
fctx_create(dns_resolver_t *res,
	    const dns_name_t *name,
	    dns_rdatatype_t type,
 ...
 */
static isc_result_t
ncache_adderesult(dns_message_t *message,
		  dns_db_t *cache,
		  dns_dbnode_t *node,
		  dns_rdatatype_t covers,
		  isc_stdtime_t now,
		  ...
		  isc_result_t *eresultp)
{
	...
	if (secure) {
		result = dns_ncache_addoptout(message,
					      cache,
					      node,
					      covers,
					      now,
...
static inline isc_result_t
ncache_message(fetchctx_t *fctx,
	       dns_adbaddrinfo_t *addrinfo,
	       dns_rdatatype_t covers,
	       isc_stdtime_t now)
{
...
static isc_result_t
check_section(void *arg,
	      const dns_name_t *addname,
	      dns_rdatatype_t type,
	      dns_section_t section)
{
	fetchctx_t *fctx = arg;
...
static bool
is_answeraddress_allowed(dns_view_t *view,
			 dns_name_t *name,
			 dns_rdataset_t *rdataset)
{
	isc_result_t result;
	     ...
	     result = dns_rdataset_next(rdataset))
	{
		...
		if (rdataset->type == dns_rdatatype_a) {
			...
			memmove(in6a.s6_addr, rdata.data,
				sizeof(in6a.s6_addr));
...
static bool
is_answertarget_allowed(fetchctx_t *fctx,
			dns_name_t *qname,
			dns_name_t *rname,
			dns_rdataset_t *rdataset,
			bool *chainingp)
...
resquery_response(isc_task_t *task, isc_event_t *event) {
	...
	if (query->tsigkey) {
		result =
			dns_message_settsigkey(fctx->rmessage, query->tsigkey);
		if (result != ISC_R_SUCCESS) {
...
rctx_answer_scan(respctx_t *rctx) {
	...
	for (result =
		     dns_message_firstname(fctx->rmessage, DNS_SECTION_ANSWER);
	     result == ISC_R_SUCCESS;
...
rctx_answer_match(respctx_t *rctx) {
	...
	(void)dns_rdataset_additionaldata(rctx->ardataset, check_related,
					  fctx);
...
rctx_ncache(respctx_t *rctx) {
	 ...
	 */
	result =
		ncache_message(fctx, rctx->query->addrinfo, covers, rctx->now);
	if (result != ISC_R_SUCCESS) {
...
void
dns_resolver_whenshutdown(dns_resolver_t *res,
			  isc_task_t *task,
			  isc_event_t **eventp)
{
	isc_task_t *tclone;
...
isc_result_t
dns_resolver_createfetch(dns_resolver_t *res,
			 const dns_name_t *name,
			 dns_rdatatype_t type,
			 const dns_name_t *domain,
			 dns_rdataset_t *nameservers,
...
void
dns_resolver_logfetch(dns_fetch_t *fetch,
		      isc_log_t *lctx,
...
isc_result_t
dns_resolver_addalternate(dns_resolver_t *resolver,
			  const isc_sockaddr_t *alt,
			  const dns_name_t *name,
			  in_port_t port) {
	alternate_t *a;
	isc_result_t result;
...
void
dns_resolver_addbadcache(dns_resolver_t *resolver,
			 const dns_name_t *name,
			 dns_rdatatype_t type,
			 isc_time_t *expire)
{
...
bool
dns_resolver_getbadcache(dns_resolver_t *resolver,
			 const dns_name_t *name,
			 dns_rdatatype_t type,
			 isc_time_t *now)
{
...
isc_result_t
dns_resolver_setmustbesecure(dns_resolver_t *resolver,
			     const dns_name_t *name,
			     bool value)
{
	isc_result_t result;
...
bool
dns_resolver_getmustbesecure(dns_resolver_t *resolver,
			     const dns_name_t *name) {
	void *data = NULL;
	bool value = false;
	isc_result_t result;
	...
	result =
		dns_rbt_findname(resolver->mustbesecure, name, 0, NULL, &data);
	if (result == ISC_R_SUCCESS || result == DNS_R_PARTIALMATCH) {
		value = *(bool*)data;
	}
 unlock:
