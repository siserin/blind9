...
isc_result_t
dns_nsec3_buildrdata(dns_db_t *db,
...
static isc_result_t
find_nsec3(dns_rdata_nsec3_t *nsec3,
	   dns_rdataset_t *rdataset,
	   const dns_rdata_nsec3param_t *nsec3param)
{
	isc_result_t result;
...
isc_result_t
dns_nsec3_addnsec3(dns_db_t *db,
		   dns_dbversion_t *version,
		   const dns_name_t *name,
		   const dns_rdata_nsec3param_t *nsec3param,
		   dns_ttl_t nsecttl,
 ...
 */
isc_result_t
dns_nsec3_addnsec3s(dns_db_t *db,
		    dns_dbversion_t *version,
		    const dns_name_t *name,
		    dns_ttl_t nsecttl,
		    bool unsecure,
...
bool
dns_nsec3param_fromprivate(dns_rdata_t *src,
			   dns_rdata_t *target,
			   unsigned char *buf,
			   size_t buflen)
{
...
void
dns_nsec3param_toprivate(dns_rdata_t *src,
			 dns_rdata_t *target,
...
isc_result_t
dns_nsec3param_salttotext(dns_rdata_nsec3param_t *nsec3param,
			  char *dst,
			  size_t dstlen)
{
	isc_result_t result;
...
isc_result_t
dns_nsec3param_deletechains(dns_db_t *db,
			    dns_dbversion_t *ver,
			    dns_zone_t *zone,
			    bool nonsec,
			    dns_diff_t *diff)
...
isc_result_t
dns_nsec3_addnsec3sx(dns_db_t *db,
		     dns_dbversion_t *version,
		     const dns_name_t *name,
		     dns_ttl_t nsecttl,
		     bool unsecure,
...
isc_result_t
dns_nsec3_delnsec3(dns_db_t *db,
		   dns_dbversion_t *version,
		   const dns_name_t *name,
		   const dns_rdata_nsec3param_t *nsec3param,
		   dns_diff_t *diff)
...
isc_result_t
dns_nsec3_delnsec3s(dns_db_t *db,
		    dns_dbversion_t *version,
		    const dns_name_t *name,
		    dns_diff_t *diff)
{
...
isc_result_t
dns_nsec3_delnsec3sx(dns_db_t *db,
		     dns_dbversion_t *version,
		     const dns_name_t *name,
		     dns_rdatatype_t privatetype,
		     dns_diff_t *diff)
{
	     ...
	     result = dns_rdataset_next(&rdataset))
	{
		 ...
		 * We have a active chain.  Update it.
		 */
		CHECK(dns_nsec3_delnsec3(db, version, name, &nsec3param,
					 diff));
	}
	dns_rdataset_disassociate(&rdataset);
...
isc_result_t
dns_nsec3_maxiterations(dns_db_t *db,
			dns_dbversion_t *version,
			isc_mem_t *mctx,
...
isc_result_t
dns_nsec3_noexistnodata(dns_rdatatype_t type,
			const dns_name_t *name,
