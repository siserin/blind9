	      ...
	      isc_result_t result)
{
	...
	if (zone->task != NULL) {
		...
		if (zone->type == dns_zone_master &&
		    ...
		    dns_zone_isdynamic(zone, false) && dns_db_issecure(db)) {
			dns_name_t *name;
			dns_fixedname_t fixed;
			dns_rdataset_t next;
	...
	while (signing != NULL && nodes-- > 0 && signatures > 0) {
		...
		for (i = 0; !has_alg && i < nkeys; i++) {
			...
			CHECK(sign_a_node(db, zone, name, node, version,
					  ...
					  inception, expire, zone->minimum,
					  is_ksk, (both && keyset_kskonly),
	...
	for (result = dns_rdataset_first(&kfetch->dnskeysigset);
		...
		while (result == ISC_R_SUCCESS) {
			...
			if (dst_key_alg(dstkey) == sig.algorithm &&
				...
				result = dns_dnssec_verify(
					keyname, &kfetch->dnskeyset, dstkey,
					false, 0, zone->view->mctx, &sigrr,
					dns_fixedname_name(&fixed));
...
zone_xfrdone(dns_zone_t *zone, isc_result_t result)
{
	...
	dns_zone_log(zone, ISC_LOG_DEBUG(1), "zone transfer finished: %s",
		     dns_result_totext(result));
	...
	case DNS_R_UPTODATE:
		...
		if (result == ISC_R_SUCCESS && xfrresult == ISC_R_SUCCESS) {
			...
			if (zone->tsigkey != NULL) {
				...
				buf[0] = '\0';
			dns_zone_log(zone, ISC_LOG_INFO,
				     "transferred serial %u%s", serial, buf);
			if (inline_raw(zone))
				zone_send_secureserial(zone, serial);
		}
	  ...
	  isc_stdtime_t now, dns_diff_t *diff, dns__zonediff_t *zonediff)
{
	...
	if (tuple == NULL) {
		result = del_sigs(zone, db, ver, &zone->origin,
				  dns_rdatatype_dnskey, zonediff, zone_keys,
				  nkeys, now, false);
		if (result != ISC_R_SUCCESS) {
			dnssec_log(zone, ISC_LOG_ERROR,
