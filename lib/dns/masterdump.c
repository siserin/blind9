...
static isc_result_t
ncache_summary(dns_rdataset_t *rdataset,
	       bool omit_final_dot,
	       isc_buffer_t *target)
{
	...
	do {
		     ...
		     result = dns_rdataset_next(&rds))
		{
			...
			if (rds.type == dns_rdatatype_rrsig) {
				CHECK(str_totext(" ", target));
				CHECK(dns_rdatatype_totext(rds.covers,
							   target));
				CHECK(str_totext(" ...\n", target));
			} else {
		...
		isc_buffer_t *target)
{
	...
	while (result == ISC_R_SUCCESS) {
		      ...
		      rdataset->ttl == current_ttl))
		{
			...
			if ((ctx->style.flags & DNS_STYLEFLAG_TTL_UNITS) !=
			    0) {
				length = target->used;
...
static isc_result_t
dump_rdatasets_text(isc_mem_t *mctx,
		    const dns_name_t *name,
		    dns_rdatasetiter_t *rdsiter,
		    dns_totext_ctx_t *ctx,
		    isc_buffer_t *buffer,
 ...
 again:
	...
	for (i = 0; i < n; i++) {
		...
		if (((rds->attributes & DNS_RDATASETATTR_NEGATIVE) != 0) &&
			...
			result =
				dump_rdataset(mctx, name, rds, ctx, buffer, f);
			if (result != ISC_R_SUCCESS) {
				dumpresult = result;
			}
 ...
 */
static isc_result_t
dump_rdataset_raw(isc_mem_t *mctx,
		  const dns_name_t *name,
		  dns_rdataset_t *rdataset,
		  isc_buffer_t *buffer,
		  FILE *f)
...
static isc_result_t
dump_rdatasets_raw(isc_mem_t *mctx,
		   const dns_name_t *owner_name,
		   dns_rdatasetiter_t *rdsiter,
		   dns_totext_ctx_t *ctx,
		   isc_buffer_t *buffer,
...
static isc_result_t
dump_rdatasets_map(isc_mem_t *mctx,
		   const dns_name_t *name,
		   dns_rdatasetiter_t *rdsiter,
		   dns_totext_ctx_t *ctx,
		   isc_buffer_t *buffer,
...
isc_result_t
dns_master_dumptostreaminc(isc_mem_t *mctx,
			   dns_db_t *db,
			   dns_dbversion_t *version,
			   const dns_master_style_t *style,
			   FILE *f,
...
isc_result_t
dns_master_dumptostream(isc_mem_t *mctx,
			dns_db_t *db,
			dns_dbversion_t *version,
			const dns_master_style_t *style,
			dns_masterformat_t format,
...
isc_result_t
dns_master_dumpinc(isc_mem_t *mctx,
		   dns_db_t *db,
		   dns_dbversion_t *version,
		   const dns_master_style_t *style,
		   const char *filename,
...
isc_result_t
dns_master_dump(isc_mem_t *mctx,
		dns_db_t *db,
		dns_dbversion_t *version,
		const dns_master_style_t *style,
		const char *filename,
 ...
 */
isc_result_t
dns_master_dumpnodetostream(isc_mem_t *mctx,
			    dns_db_t *db,
			    dns_dbversion_t *version,
			    dns_dbnode_t *node,
			    const dns_name_t *name,
...
isc_result_t
dns_master_dumpnode(isc_mem_t *mctx,
		    dns_db_t *db,
		    dns_dbversion_t *version,
		    dns_dbnode_t *node,
		    const dns_name_t *name,
