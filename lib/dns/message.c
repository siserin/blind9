...
static void
logfmtpacket(dns_message_t *message,
...
isc_result_t
dns_message_find(const dns_name_t *name,
...
isc_result_t
dns_message_findtype(const dns_name_t *name,
		     dns_rdatatype_t type,
		     dns_rdatatype_t covers,
		     dns_rdataset_t **rdataset)
{
...
static isc_result_t
getsection(isc_buffer_t *source,
	   dns_message_t *msg,
	   dns_decompress_t *dctx,
...
isc_result_t
dns_message_parse(dns_message_t *msg,
		  isc_buffer_t *source,
		  unsigned int options)
{
	isc_region_t r;
...
isc_result_t
dns_message_renderbegin(dns_message_t *msg,
			dns_compress_t *cctx,
			isc_buffer_t *buffer)
{
	isc_region_t r;
...
static isc_result_t
renderset(dns_rdataset_t *rdataset,
...
isc_result_t
dns_message_rendersection(dns_message_t *msg,
			  dns_section_t sectionid,
			  unsigned int options)
{
	dns_namelist_t *section;
...
void
dns_message_currentname(dns_message_t *msg,
...
isc_result_t
dns_message_findname(dns_message_t *msg,
		     dns_section_t section,
		     const dns_name_t *target,
		     dns_rdatatype_t type,
		     dns_rdatatype_t covers,
...
void
dns_message_movename(dns_message_t *msg,
		     dns_name_t *name,
...
void
dns_message_removename(dns_message_t *msg,
		       dns_name_t *name,
		       dns_section_t section)
{
	REQUIRE(msg != NULL);
...
isc_result_t
dns_message_peekheader(isc_buffer_t *source,
		       dns_messageid_t *idp,
		       unsigned int *flagsp)
{
	isc_region_t r;
...
isc_result_t
dns_message_getquerytsig(dns_message_t *msg,
			 isc_mem_t *mctx,
...
isc_result_t
dns_message_sectiontotext(dns_message_t *msg,
			  dns_section_t section,
			  const dns_master_style_t *style,
	...
	do {
		     ...
		     rdataset = ISC_LIST_NEXT(rdataset, link))
		{
			if (section == DNS_SECTION_ANSWER &&
				...
				if (seensoa &&
				    (flags & DNS_MESSAGETEXTFLAG_ONESOA) !=
				    0) {
					continue;
				}
				seensoa = true;
...
isc_result_t
dns_message_totext(dns_message_t *msg,
		   const dns_master_style_t *style,
		   dns_messagetextflag_t flags,
		   isc_buffer_t *target)
{
...
void
dns_message_setsortorder(dns_message_t *msg,
...
isc_result_t
dns_message_buildopt(dns_message_t *message,
