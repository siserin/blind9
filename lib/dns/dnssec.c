	...
	*nkeys = 0;
	...
	while (result == ISC_R_SUCCESS && count < maxkeys) {
		...
		result = dst_key_fromfile(
			...
			dst_key_alg(pubkey), DST_TYPE_PUBLIC | DST_TYPE_PRIVATE,
			    ...
			    dns_dnsseckeylist_t *keylist)
{
	...
	while (isc_dir_read(&dir) == ISC_R_SUCCESS) {
		...
		result = dst_key_fromnamedfile(
			...
			DST_TYPE_PUBLIC | DST_TYPE_PRIVATE, mctx, &dstkey);
...
static isc_result_t
publish(dns_rdata_t *rdata, dns_diff_t *diff, dns_name_t *origin, dns_ttl_t ttl,
	isc_mem_t *mctx)
{
	isc_result_t result;
	    ...
	    void (*report)(const char *, ...))
{
	...
	/* publish key */
	RETERR(dns_difftuple_create(mctx, DNS_DIFFOP_ADD, origin, ttl, &dnskey,
				    &tuple));
	dns_diff_appendminimal(diff, &tuple);
	result = ISC_R_SUCCESS;
	...
	for (key = ISC_LIST_HEAD(*keys); key != NULL;
		...
		if (syncpublish(key->key, now)) {
			if (!dns_rdataset_isassociated(cdnskey) ||
				...
				RETERR(publish(&cdnskeyrdata, diff, origin, ttl,
					       mctx));
			}
			/* Only publish SHA-256 (SHA-1 is deprecated) */
			if (!dns_rdataset_isassociated(cds) ||
			    !exists(cds, &cds_sha256)) {
				RETERR(publish(&cds_sha256, diff, origin, ttl,
					       mctx));
			}
		}
		      ...
		      isc_mem_t *mctx, void (*report)(const char *, ...))
{
	...
	for (key = ISC_LIST_HEAD(*keys); key != NULL;
		...
		if (key->source == dns_keysource_user &&
			...
			RETERR(publish_key(diff, key, origin, ttl, mctx, allzsk,
					   report));
		}
