...
static void
limit_ttl(dns_rdatacallbacks_t *callbacks,
	  const char *source,
	  unsigned int line,
	  uint32_t *ttlp)
{
...
static void
check_wildcard(dns_incctx_t *ictx,
	       const char *source,
	       unsigned long line,
	       dns_rdatacallbacks_t *callbacks)
{
	...
	do {
			   ...
			   token.type == isc_tokentype_qstring)
		{
			...
			if (finish_include) {
				...
				EXPECTEOL;
				result =
					pushfile(include_file, new_name, lctx);
				if (MANYERRS(lctx, result)) {
					SETRESULT(lctx, result);
		...
		if (!lctx->ttl_known && !lctx->default_ttl_known) {
			if (type == dns_rdatatype_soa) {
				(*callbacks->warn)(callbacks,
						   ...
						   source, line);
				lctx->ttl =
					dns_soa_getminimum(&rdata[rdcount]);
				limit_ttl(callbacks, source, line, &lctx->ttl);
				lctx->default_ttl = lctx->ttl;
				lctx->default_ttl_known = true;
...
pushfile(const char *master_file, dns_name_t *origin, dns_loadctx_t *lctx) {
	...
	if (ictx->glue != NULL || ictx->current != NULL) {
		...
		newctx->current =
			dns_fixedname_name(&newctx->fixed[newctx->
							  current_in_use]);
 ...
 */
static dns_rdatalist_t *
grow_rdatalist(int new_len,
	       dns_rdatalist_t *oldlist,
	       int old_len,
...
static isc_result_t
commit(dns_rdatacallbacks_t *callbacks,
       dns_loadctx_t *lctx,
       rdatalist_head_t *head,
       dns_name_t *owner,
       const char *source,
