...
static isc_result_t
create_tcp_dispatch(bool newtcp,
		    bool share,
		    dns_requestmgr_t *requestmgr,
...
static isc_result_t
find_udp_dispatch(dns_requestmgr_t *requestmgr,
		  const isc_sockaddr_t *srcaddr,
		  const isc_sockaddr_t *destaddr,
		  dns_dispatch_t **dispatchp)
{
...
set_timer(isc_timer_t *timer, unsigned int timeout, unsigned int udpresend) {
	...
	timertype = udpresend !=
		    0 ? isc_timertype_limited : isc_timertype_once;
	if (result == ISC_R_SUCCESS) {
...
static isc_result_t
req_render(dns_message_t *message,
	   isc_buffer_t **bufferp,
	   unsigned int options,
	   isc_mem_t *mctx)
{
