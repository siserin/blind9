 ...
 */
isc_result_t
dns_dnsrps_rewrite_init(librpz_emsg_t *emsg,
			dns_rpz_st_t *st,
...
static void
rpsdb_bind_rdataset(dns_rdataset_t *rdataset,
		    uint count,
		    librpz_idx_t next_rr,
		    dns_rdatatype_t type,
		    uint16_t class,
 ...
 */
static isc_result_t
rpsdb_findrdataset(dns_db_t *db,
		   dns_dbnode_t *node,
...
static isc_result_t
rpsdb_allrdatasets(dns_db_t *db,
		   dns_dbnode_t *node,
		   dns_dbversion_t *version,
		   isc_stdtime_t now,
