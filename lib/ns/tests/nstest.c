...
static isc_result_t
matchview(isc_netaddr_t*srcaddr,
 ...
 */
isc_result_t
ns_test_makezone(const char*name,
		 dns_zone_t**zonep,
		 dns_view_t*view,
		 bool keepview)
{
 ...
 */
static isc_result_t
attach_query_msg_to_client(ns_client_t*client,
			   const char*qnamestr,
...
isc_result_t
ns_test_getdata(const char*file,unsigned char*buf,size_t bufsiz,size_t*sizep)
{
	isc_result_t result;
	unsigned char*bp;
	char*rp,*wp;
