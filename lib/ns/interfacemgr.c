...
ns_interface_listenudp(ns_interface_t *ifp)
{
	...
	for (disp = 0; disp < ifp->nudpdispatch; disp++) {
		result = dns_dispatch_getudp_dup(
			ifp->mgr->dispatchmgr, ifp->mgr->socketmgr,
			ifp->mgr->taskmgr, &ifp->addr, 4096, UDPBUFFERS, 32768,
			8219, 8237, attrs, attrmask, &ifp->udpdispatch[disp],
			disp == 0 ? NULL : ifp->udpdispatch[0]);
		if (result != ISC_R_SUCCESS) {
			isc_log_write(IFMGR_COMMON_LOGARGS, ISC_LOG_ERROR,
...
ns_interface_accepttcp(ns_interface_t *ifp)
{
	...
	result = isc_socket_create(ifp->mgr->socketmgr,
				   ...
				   isc_sockettype_tcp, &ifp->tcpsocket);
	if (result != ISC_R_SUCCESS) {
		isc_log_write(IFMGR_COMMON_LOGARGS, ISC_LOG_ERROR,
