...
static bool
rpz_ck_dnssec(ns_client_t*client,
...
static void
recparam_update(ns_query_recparam_t*param,
		dns_rdatatype_t qtype,
...
static isc_result_t
query_addsoa(query_ctx_t*qctx,unsigned int override_ttl,dns_section_t section);
 ...
 */
static isc_result_t
query_checkcacheaccess(ns_client_t*client,
...
query_validatezonedb(ns_client_t*client,
		     const dns_name_t*name,
		     dns_rdatatype_t qtype,
		     unsigned int options,
		     dns_zone_t*zone,
...
static inline isc_result_t
query_getzonedb(ns_client_t*client,
		const dns_name_t*name,
		dns_rdatatype_t qtype,
		unsigned int options,
		dns_zone_t**zonep,
...
static void
rpz_log_rewrite(ns_client_t*client,
		bool disabled,
...
static void
rpz_log_fail_helper(ns_client_t*client,
		    int level,
		    dns_name_t*p_name,
		    dns_rpz_type_t rpz_type1,
		    dns_rpz_type_t rpz_type2,
		    const char*str,
		    isc_result_t result)
{
...
static void
rpz_log_fail(ns_client_t*client,
	     int level,
	     dns_name_t*p_name,
 ...
 */
static inline isc_result_t
query_getcachedb(ns_client_t*client,
		 const dns_name_t*name,
		 dns_rdatatype_t qtype,
		 dns_db_t**dbp,
		 unsigned int options)
...
static inline isc_result_t
query_getdb(ns_client_t*client,
	    dns_name_t*name,
	    dns_rdatatype_t qtype,
	    unsigned int options,
	    dns_zone_t**zonep,
...
static inline bool
query_isduplicate(ns_client_t*client,
		  dns_name_t*name,
 ...
 */
static isc_result_t
query_additionalauthfind(dns_db_t*db,
			 dns_dbversion_t*version,
			 const dns_name_t*name,
			 dns_rdatatype_t type,
			 ns_client_t*client,
 ...
 */
static isc_result_t
query_additionalauth(query_ctx_t*qctx,
		     const dns_name_t*name,
		     dns_rdatatype_t type,
		     dns_db_t**dbp,
		     dns_dbnode_t**nodep,
...
static void
query_addrrset(query_ctx_t*qctx,
	       dns_name_t**namep,
 ...
 */
static void
mark_secure(ns_client_t*client,
	    dns_db_t*db,
	    dns_name_t*name,
...
static void
free_devent(ns_client_t*client,isc_event_t**eventp,dns_fetchevent_t**deventp)
{
 ...
 */
static isc_result_t
rpz_rrset_find(ns_client_t*client,
	       dns_name_t*name,
	       dns_rdatatype_t type,
	       dns_rpz_type_t rpz_type,
	       dns_db_t**dbp,
 ...
 */
static isc_result_t
rpz_get_p_name(ns_client_t*client,
	       dns_name_t*p_name,
	       dns_rpz_zone_t*rpz,
	       dns_rpz_type_t rpz_type,
 ...
 */
static int
dnsrps_ck(librpz_emsg_t*emsg,ns_client_t*client,rpsdb_t*rpsdb,bool recursed)
{
	isc_region_t region;
 ...
 */
static bool
dnsrps_set_p(librpz_emsg_t*emsg,
	     ns_client_t*client,
	     dns_rpz_st_t*st,
	     dns_rdatatype_t qtype,
...
static isc_result_t
dnsrps_rewrite_ip(ns_client_t*client,
...
static isc_result_t
dnsrps_rewrite_name(ns_client_t*client,
		    dns_name_t*trig_name,
		    bool recursed,
		    dns_rpz_type_t rpz_type,
		    dns_rdataset_t**p_rdatasetp)
 ...
 */
static isc_result_t
rpz_rewrite_ip(ns_client_t*client,
	       const isc_netaddr_t*netaddr,
	       dns_rdatatype_t qtype,
	       dns_rpz_type_t rpz_type,
 ...
 */
static isc_result_t
rpz_rewrite_ip_rrsets(ns_client_t*client,
		      dns_name_t*name,
		      dns_rdatatype_t qtype,
		      dns_rpz_type_t rpz_type,
		      dns_rdataset_t**ip_rdatasetp,
 ...
 */
static isc_result_t
rpz_rewrite_name(ns_client_t*client,
		 dns_name_t*trig_name,
		 dns_rdatatype_t qtype,
		 dns_rpz_type_t rpz_type,
...
static void
rpz_rewrite_ns_skip(ns_client_t*client,
		    dns_name_t*nsname,
		    isc_result_t result,
		    int level,
		    const char*str)
 ...
 */
static isc_result_t
rpz_rewrite(ns_client_t*client,
	    dns_rdatatype_t qtype,
	    isc_result_t qresult,
	    bool resuming,
	    dns_rdataset_t*ordataset,
	    dns_rdataset_t*osigset)
{
	dns_rpz_zones_t*rpzs;
	dns_rpz_st_t*st;
	dns_rdataset_t*rdataset;
	    ...
	    (DNS_RPZ_DONE_CLIENT_IP | DNS_RPZ_DONE_QNAME))
	{
		...
		if ((st->state & DNS_RPZ_DONE_QNAME) == 0) {
			 ...
			 */
			st->r.label =
				dns_name_countlabels(client->query.qname);
	 ...
	 dns_rdatatype_t qtype)
{
	...
	result = ns_client_checkaclsilent(client,NULL,
					  dns_zone_getqueryacl(client->view->
							       redirect),
					  true);
	if (result != ISC_R_SUCCESS) {
		return (ISC_R_NOTFOUND);
 ...
 */
static bool
recparam_match(const ns_query_recparam_t*param,
	       dns_rdatatype_t qtype,
	       const dns_name_t*qname,
	       const dns_name_t*qdomain)
{
...
isc_result_t
ns_query_recurse(ns_client_t*client,
		 dns_rdatatype_t qtype,
		 dns_name_t*qname,
		 dns_name_t*qdomain,
		...
		if  (result == ISC_R_SOFTQUOTA) {
			...
			if (now != atomic_load_relaxed(&last)) {
				...
				ns_client_log(client,
					      ...
					      isc_quota_getused(client->
								recursionquota),
					      isc_quota_getsoft(client->
								recursionquota),
					      isc_quota_getmax(client->
							       recursionquota));
			}
			...
			if (now != atomic_load_relaxed(&last)) {
				...
				ns_client_log(client,
					      ...
					      "(%u/%u/%u): %s",
					      isc_quota_getused(&sctx->
								recursionquota),
					      isc_quota_getsoft(&sctx->
								recursionquota),
					      isc_quota_getmax(&sctx->
							       recursionquota),
					      isc_result_totext(result));
			}
			ns_client_killoldestquery(client);
...
static dns_ttl_t
query_synthttl(dns_rdataset_t*soardataset,
	       dns_rdataset_t*sigsoardataset,
	       dns_rdataset_t*p1rdataset,
	       dns_rdataset_t*sigp1rdataset,
	       dns_rdataset_t*p2rdataset,
	       dns_rdataset_t*sigp2rdataset)
{
	dns_rdata_soa_t soa;
	dns_rdata_t rdata = DNS_RDATA_INIT;
	dns_ttl_t ttl;
 ...
 */
static isc_result_t
query_synthnodata(query_ctx_t*qctx,
 ...
 */
static isc_result_t
query_synthwildcard(query_ctx_t*qctx,
		    dns_rdataset_t*rdataset,
		    dns_rdataset_t*sigrdataset)
{
	dns_name_t*name = NULL;
 ...
 */
static isc_result_t
query_synthcnamewildcard(query_ctx_t*qctx,
			 dns_rdataset_t*rdataset,
			 dns_rdataset_t*sigrdataset)
{
	isc_result_t result;
...
ns_query_done(query_ctx_t*qctx) {
	...
	if (qctx->want_restart &&
	    qctx->client->query.restarts < MAX_RESTARTS) {
