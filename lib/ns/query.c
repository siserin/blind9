...
query_lookup(query_ctx_t *qctx)
{
	...
	if (qctx->dns64 && qctx->rpz) {
		...
		rresult = dns_name_copy(qctx->client->query.qname, qctx->fname,
					NULL);
...
query_resume(query_ctx_t *qctx)
{
	...
	if (result != ISC_R_SUCCESS) {
		CCTRACE(ISC_LOG_ERROR, "query_resume: dns_name_copy failed");
		QUERY_ERROR(qctx, result);
		return (ns_query_done(qctx));
	}
...
query_dns64(query_ctx_t *qctx)
{
	...
	result = isc_buffer_allocate(
		client->mctx, &buffer,
		view->dns64cnt * 16 * dns_rdataset_count(qctx->rdataset));
	if (result != ISC_R_SUCCESS)
		goto cleanup;
