...
static isc_result_t get_worker(ns_clientmgr_t*manager,
			       ns_interface_t*ifp,
			       isc_socket_t*sock);
static void compute_cookie(ns_client_t*client,
			   uint32_t when,
			   uint32_t nonce,
...
static isc_result_t
client_allocsendbuf(ns_client_t*client,
		    isc_buffer_t*buffer,
...
client_send(ns_client_t*client) {
	...
	if (client->view != NULL) {
		...
		} else if (client->view->preferred_glue ==
			   dns_rdatatype_aaaa)
		{
...
static void
compute_cookie(ns_client_t*client,
	       uint32_t when,
	       uint32_t nonce,
	       const unsigned char*secret,
	       isc_buffer_t*buf)
...
static isc_result_t
get_client(ns_clientmgr_t*manager,
	   ns_interface_t*ifp,
	   dns_dispatch_t*disp,
	   bool tcp)
{
...
ns_client_dumprecursing(FILE*f,ns_clientmgr_t*manager) {
	...
	while (client != NULL) {
		...
		if (rdataset == NULL && client->query.origqname != NULL) {
			rdataset =
				ISC_LIST_HEAD(client->query.origqname->list);
		}
		if (rdataset != NULL) {
