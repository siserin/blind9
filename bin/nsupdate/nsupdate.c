...
static void
sendrequest(isc_sockaddr_t *destaddr,
	    dns_message_t *msg,
...
static void
send_gssrequest(isc_sockaddr_t *destaddr,
		dns_message_t *msg,
		dns_request_t **request,
		gss_ctx_id_t context);
static void
...
static bool
parse_hmac(const dns_name_t **hmac,
	   const char *hmacstr,
	   size_t len,
...
setup_system(void) {
	...
	if (local_only || ISC_LIST_EMPTY(*nslist)) {
		...
		ns_total = ns_alloc = (have_ipv4 ? 1 : 0) +
				      (have_ipv6 ? 1 : 0);
		servers =
			isc_mem_get(gmctx, ns_alloc * sizeof(isc_sockaddr_t));
		if (servers == NULL) {
			fatal("out of memory");
		}
