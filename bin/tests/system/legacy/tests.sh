...
ret=0
sent=`grep "sending packet to 10.53.0.7" ns1/named.run | wc -l`
if [ $sent -ge 10 ]; then
...
timeouts=`sed -n "s|.*10\.53\.0\.7.*\[edns \([0-9/][0-9/]*\).*|\1|p" ns1/named_dump.db.test$n`
timeouts512=`echo "$timeouts" | awk -F/ '{print $NF}'`
