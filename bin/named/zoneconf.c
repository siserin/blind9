 ...
 */
static isc_result_t
configure_zone_acl(const cfg_obj_t *zconfig,
 ...
 */
static isc_result_t
configure_zone_ssutable(const cfg_obj_t *zconfig,
			dns_zone_t *zone,
			const char *zname)
{
 ...
 */
static isc_result_t
configure_staticstub_serveraddrs(const cfg_obj_t *zconfig,
				 dns_zone_t *zone,
 ...
 */
static isc_result_t
configure_staticstub_servernames(const cfg_obj_t *zconfig,
				 dns_zone_t *zone,
				 dns_rdatalist_t *rdatalist,
				 const char *zname)
{
 ...
 */
static isc_result_t
configure_staticstub(const cfg_obj_t *zconfig,
		     dns_zone_t *zone,
		     const char *zname,
		     const char *dbtype)
{
 ...
 */
static bool
isself(dns_view_t *myview,
       dns_tsigkey_t *mykey,
       const isc_sockaddr_t *srcaddr,
       const isc_sockaddr_t *dstaddr,
...
process_notifytype(dns_notifytype_t ntype,
...
isc_result_t
named_zone_configure(const cfg_obj_t *config,
		     const cfg_obj_t *vconfig,
...
#define SIGNED ".signed"
	...
	if (ztype == dns_zone_master || raw != NULL) {
		...
		obj = NULL;
		result =
			named_config_get(maps, "sig-signing-signatures", &obj);
		INSIST(result == ISC_R_SUCCESS && obj != NULL);
	...
	if (ztype == dns_zone_master) {
		...
		dns_zone_setoption(mayberaw, DNS_ZONEOPT_IGNOREMXCNAME,
				   ignore);
	...
	case dns_zone_redirect:
		...
		obj = NULL;
		result =
			named_config_get(maps, "alt-transfer-source-v6", &obj);
		INSIST(result == ISC_R_SUCCESS && obj != NULL);
