...
assertion_failed(const char *file,
		 int line,
...
ISC_PLATFORM_NORETURN_PRE static void
library_fatal_error(const char *file,
		    int line,
		    const char *format,
		    va_list args)
...
static void
library_unexpected_error(const char *file,
			 int line,
			 const char *format,
...
printversion(bool verbose) {
	...
	printf("linked to libjson-c version: %s\n",
	       json_c_version());
#endif /* if defined(HAVE_JSON) && defined(JSON_C_VERSION) */
