...
setup_text_key(void) {
	...
	result =
		dns_name_fromtext(&keyname, namebuf, dns_rootname, 0, namebuf);
	if (result != ISC_R_SUCCESS) {
		goto failure;
	}
...
static isc_result_t
parse_uint_helper(uint32_t *uip,
		  const char *value,
		  uint32_t max,
		  const char *desc,
		  int base) {
 ...
 */
static void
add_question(dns_message_t *message,
	     dns_name_t *name,
...
#ifdef HAVE_LIBIDN2
	if (lookup->idnin) {
		idn_locale_to_ace(textname, idn_textname,
				  sizeof(idn_textname));
...
bringup_timer(dig_query_t *query, unsigned int default_timeout) {
	 ...
	 */
	l = query->lookup;
	if (ISC_LINK_LINKED(query,
			    link) && ISC_LIST_NEXT(query, link) != NULL) {
 ...
 */
static bool
check_for_more_data(dig_query_t *query,
		    dns_message_t *msg,
		    isc_socketevent_t *sevent)
{
	dns_rdataset_t *rdataset = NULL;
