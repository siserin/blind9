...
in ``named.conf`` using a ``dnssec-keys`` statement (or the synonymous
``managed-keys`` or the deprecated ``trusted-keys`` statements).
...
The keys specified in ``dnssec-keys`` copies of DNSKEY RRs for zones
that are used to form the first link in the cryptographic chain of
